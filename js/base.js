...
function initDemoApp() {
    ...
    webim.getPendencyGroup({
        ...
        for (var i in pendency.PendencyList) {
            pendency.PendencyList[i].Operator_Account = pendency.PendencyList[i].From_Account;
            pendency.PendencyList[i].RemarkInfo = pendency.PendencyList[i].ApplyInviteMsg;
            pendency.PendencyList[i].MsgTime = (pendency.PendencyList[i].AddTime) / 1000;
            pendency.PendencyList[i].Handled = pendency.PendencyList[i].Handled == 0 ? '未处理' : '已处理';
            pendency.PendencyList[i].groupReportTypeMsg = {};
            pendency.PendencyList[i].groupReportTypeMsg.Authentication = pendency.PendencyList[i].Authentication;
            pendency.PendencyList[i].groupReportTypeMsg.MsgKey = pendency.PendencyList[i].MsgKey;
            pendency.PendencyList[i].groupReportTypeMsg.Operator_Account = pendency.PendencyList[i].From_Account;
            pendency.PendencyList[i].groupReportTypeMsg.RemarkInfo = pendency.PendencyList[i].ApplyInviteMsg;
            pendency.PendencyList[i].groupReportTypeMsg.ReportType = pendency.PendencyList[i].PendencyType;
            var list = pendency.PendencyList[i];
            onApplyJoinGroupRequestNotify(list)
        }
