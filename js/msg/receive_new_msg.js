...
function onMsgNotify(newMsgList) {
    ...
    if (pagename == 'index_yqq') {
        for (var i = 0; i < newMsgList.length; i++) {
            if (newMsgList[i].isSend) {
                newMsgList[i].accountImg = loginInfo.headurl;
            } else {
                for (var j = 0; j < infoMap.length; j++) {
                    if (newMsgList[i].fromAccount == infoMap[j].id) {
                        newMsgList[i].accountImg = infoMap[j].image;
                    }
                }
            }
            var sess = newMsgList[i].sess;
            //更新其他聊天对象的未读消息数
            updateSessDiv(sess.id(), sess.unread());
            msgListShow(newMsgList[i]);
        }
    } else {
