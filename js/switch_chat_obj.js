...
function addSess(sess_type, to_id, name, face_url, unread_msg_count, sesslist, addPositonType) {
    ...
    sessDiv.onclick = function() {
        ...
        //infoMapCS = webim.MsgStore.sessMap();
        onSelSess(sess_type, to_id) //, infoMapCS);
    };
...
function onSelSess(sess_type, to_id) {
    if (selToID != null) {
        ...
        var sessCS;
        if (sess_type == "C2C") {
            sessCS = webim.SESSION_TYPE.C2C + selToID;
        } else if (sess_type == "GROUP") {
            sessCS = webim.SESSION_TYPE.GROUP + selToID;
        }
