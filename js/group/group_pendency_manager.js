...
window.gajgpOperateEvents = {
    'click .edit': function(e, value, row, index) {
        if (row.PendencyType == 0 || row.ReportType == 1) {
        ...
        } else if (row.PendencyType == 1) {
            $("#hijg_group_id").val(row.GroupId);
            $("#hijg_to_account").val(row.Operator_Account);
            $("#hijg_msg_key").val(row.MsgKey);
            $("#hijg_authentication").val(row.Authentication);
            $("#hijg_user_defined_field").val(row.UserDefinedField);

            $("#hijg_from_account").val(row.From_Account);
            $("#hijg_msg_seq").val(row.MsgSeq);
            $("#hijg_msg_random").val(row.MsgRandom);
...
//处理邀请进群申请
var handleInviteJoinGroupRequest = function() {

    var options = {
        'GroupId': $("#hijg_group_id").val(), //群id
        'Inviter_Account': $("#hijg_to_account").val(), //邀请人id
        'HandleMsg': $('input[name="hijg_action_radio"]:checked').val(), //Agree-同意 Reject-拒绝
        'Authentication': $("#hijg_authentication").val(), //申请凭证
        'MsgKey': 0, //$("#hijg_msg_key").val(),
        'ApprovalMsg': $("#hijg_approval_msg").val(), //处理附言
        'UserDefinedField': $("#hijg_group_id").val() //用户自定义字段
    };

    //要删除的群未决消息
    var delApplyJoinGroupPendencys = {
        'DelMsgList': [{
            "From_Account": $("#hijg_from_account").val(),
            "MsgSeq": parseInt($("#hijg_msg_seq").val()),
            "MsgRandom": parseInt($("#hijg_msg_random").val())
        }]
    };


    webim.handleInviteJoinGroupRequest(
        options,
        function(resp) {
            //在表格中删除对应的行
            $('#get_apply_join_group_pendency_table').bootstrapTable('remove', {
                field: 'Authentication',
                values: [$("#hijg_authentication").val()]
            });
            $('#handle_invite_join_group_request').modal('hide');
