...
function onInvitedJoinGroupNotifyRequest(notify) {
    ...
    notify.MsgTimeStamp = timestamp;
    notify.MsgTime = webim.Tool.formatTimeStamp(notify.MsgTime);
    $('#hijg_authentication').val(notify.groupReportTypeMsg.Authentication)
    $('#hijg_group_id').val(notify.GroupId);
    $('#hijg_to_account').val(notify.Operator_Account);
    $('#hijg_msg_key').val(notify.groupReportTypeMsg.MsgKey);
...
function onInviteJoinRejected(notify) {
    console.info('同意', notify)
    webim.Log.info("执行 被邀请加群  回调: " + JSON.stringify(notify));
    //刷新我的群组列表
    //getJoinedGroupListHigh(getGroupsCallbackOK);
    var reportTypeCh = "[同意邀请加群]";
    var content = notify.Operator_Account + "被管理员邀请加入该群";
    addGroupSystemMsg(notify.ReportType, reportTypeCh, notify.GroupId, notify.GroupName, content, notify.MsgTime);
}
//监听 邀请用户进群拒绝 系统消息

function onInviteJoinAccepted(notify) {
    console.info('拒绝', notify)
    webim.Log.info("执行 被邀请加群  回调: " + JSON.stringify(notify));
    //刷新我的群组列表
    //getJoinedGroupListHigh(getGroupsCallbackOK);
    var reportTypeCh = "[拒绝邀请加群]";
    var content = notify.Operator_Account + '拒绝你的邀请'
    addGroupSystemMsg(notify.ReportType, reportTypeCh, notify.GroupId, notify.GroupName, content, notify.MsgTime);
}
